...
//my$("bymyCar").onmouseover=function(){
//	my$("fux").style.display="block";
//}
//my$("bymyCar").onmouseout=function(){
...
window.onload =function(){
	var faUl = document.getElementById("uls");  //导航标签ul
	var faLi = faUl.getElementsByTagName("li");  //导航标签ul里的li
	var tabson = document.getElementsByClassName("tabright"); //子页面盒子
	var tabAllSon = tabson[0].getElementsByTagName("div");  //全部子页面
	
	for(var i =0;i<faLi.length;i++){
		faLi[i].index = i;
		
		faLi[i].addEventListener("mouseover",function(event){
			var evt = event || window.event;
			
			for(var j=0;j<tabAllSon.length;j++){
				tabAllSon[j].className = "hide";
			}
			tabAllSon[this.index].className = "show";
			
			tabAllSon[this.index].addEventListener("mouseover",function(event){
				var evt = event || window.event;
				this.className = "show";
				evt.stopPropagation();
			},false);
			
			tabAllSon[this.index].addEventListener("mouseout",function(event){
				var evt = event || window.event;
				this.className = "hide";
				evt.stopPropagation();
			},false);
//			tabAllSon[this.index].onmouseover=function(){
//				this.className = "show";
//			}
//			tabAllSon[this.index].onmouseout=function(){
//				this.className = "hide";
//			}
			evt.stopPropagation();
		},false);
		...
		faLi[i].addEventListener("mouseout",function(event){
			var evt = event || window.event;
			for(var j = 0;j<tabAllSon.length;j++){
	        		tabAllSon[j].className = "hide";
	        	}
...
//		width:500,
//		height:200,
		imgs:["img/201931029live2pc.jpg","img/2019310302.jpg","img/20193235040h5.jpg"],
		timeSpace:3000
	},my$("banners"));
